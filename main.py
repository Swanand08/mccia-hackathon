...
def main():
    ...
    print("  Simulation Date: 2023-12-01")
    print("=" * 80)

    # ---- Step 0: Generate data if not present ----
    if not os.path.exists(os.path.join(DATA_DIR, "material_master.csv")):
        print("\n  Generating data files...")
        from generate_data import DATA_DIR as _
        print("  Data generated.")

    # ---- Load CSVs ----
    print("\n  Loading CSV data...")
    inv_txn = pd.read_csv(os.path.join(DATA_DIR, "inventory_transactions.csv"))
    ...
    forecast_df, explosion_df, upcoming_orders = run_forecast(DATA_DIR)
    ...
    procurement_df = run_procurement_engine(forecast_df, mat_master, sup_master)
